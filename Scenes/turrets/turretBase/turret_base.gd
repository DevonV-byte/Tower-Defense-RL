...
func _process(_delta):
	if not deployed:
		...
		var placement_manager = get_node("/root/main/TurretPlacementManager")
		if placement_manager:
			snapped_position = placement_manager.get_closest_valid_position(position)
			position = snapped_position
			
		# Check both position validity and collision
		can_place = not $CollisionArea.has_overlapping_areas()
		if placement_manager:
			can_place = can_place and placement_manager.is_valid_position(snapped_position)
...
func build():
	...
	var placement_manager = get_node("/root/main/TurretPlacementManager")
	if placement_manager:
		placement_manager.occupy_position(position)
...
# Called when the turret is being removed/destroyed
func _exit_tree():
	if deployed:
		var placement_manager = get_node("/root/main/TurretPlacementManager")
		if placement_manager:
			placement_manager.free_position(position)
