...
func _ready():
	Globals.mineralsChanged.connect(check_can_purchase)

func _gui_input(event):
	if event is InputEventMouseButton and check_can_purchase(Globals.currentMap.minerals):
...
func _get_drag_data(_at_position):
	if check_can_purchase(Globals.currentMap.minerals):
		visible = false
		create_placeholder()
...
func build():
	Globals.currentMap.minerals -= Data.turrets[turretType]["cost"]
	placeholder.build()
