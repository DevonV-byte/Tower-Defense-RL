...
func _ready():
	Globals.mineralsChanged.connect(check_can_upgrade)
...
func _on_upgrade_button_pressed():
	if check_can_upgrade():
		Globals.currentMap.minerals -= get_upgrade_price()
...
func get_sell_price():
	...
	# Basic turret gets 100% refund, elementals get 75%
	var modifier = 1.0 if turret.turret_type == "basic" else 0.75
	return round(total_cost * modifier)
...
func check_can_upgrade(_new_gold=0):
	if turret.turret_level == Data.turrets[turret.turret_type]["max_level"]:
		...
		%UpgradeButton.disabled = Globals.currentMap.minerals < get_upgrade_price()
	return not %UpgradeButton.disabled


func _on_sell_button_pressed():
	queue_free()
	Globals.currentMap.minerals += get_sell_price()
	turret.queue_free()
